...
**Ongoing (still reproducing identically despite the above):** the `ConnectionResetByPeer` still occurred at the same byte offset in a third log, which rules out SIGPIPE, stdin-EOF, and framing (the `length` field is total packet length, min 5, so the client's `pkt_len < 5` exit can't fire on valid server packets). Remaining suspects are signal-driven death (SIGHUP on terminal hangup, SIGTTOU/SIGTTIN from job control) or a silent panic — none of which the client logged. Hardened the client and added exit diagnostics: ignore SIGTTOU/SIGTTIN (tmux practice), handle SIGHUP non-fatally (flag + log, stay attached), and log the startup + every exit path (server_fd error/EOF, malformed packet, `.detach`, propagated errors via `main`'s catch). The next reproduction should say exactly which path kills the client.
