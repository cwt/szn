...
**Follow-up (single oversized image freezes szn):** the trigger was a specific file — a 1024×1536 PNG whose native-resolution sixel is ~2.6 MiB. On a wide terminal its footprint (77×103 cells) fits the grid, so it gets placed, but the render emits a 2.6 MiB frame that exceeds `MAX_OUT_BUF` (1 MiB) and is dropped, resetting `last_cells`; the server then re-renders and re-drops it forever — the freeze/display-death. Fix: `Screen.MAX_SIXEL_DATA = 768 KiB` — `addSixelImage` drops any sixel whose raw DCS exceeds it (logged), so an oversized frame is never generated. Regression test: an oversized sixel is dropped and not buffered.
