...
**Live-inspection finding (mosh is the bottleneck):** a live frozen session showed: mosh-server idle at 0.3% CPU (not draining the pty; its UDP socket is a tailscale IP), the client's stdout pty full with `out_buf` stuck ≥ the watermark (so it stops reading the server socket — 199 KiB unread), opencode never rendering (no alt-screen entry, no row paint — the pane was throttled before its render), and the server busy-spinning at 96.9% CPU on the unread pane data. The backpressure chain (local mosh-client/network not consuming → mosh-server stops draining the pty → client congested → `DisplayClient.behind` → pane throttled → opencode frozen) is szn working as designed; the root cause is the mosh link, not szn. Also found: the server queued 159,400 × 5-byte packets (via `appendClientOut`) that are not output frames — source under investigation via new per-message-type counters. Fixed the 96.9% CPU busy-spin (stop polling the pane while behind; re-arm on drain).
