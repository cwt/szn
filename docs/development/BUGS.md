...
**ROOT CAUSE FOUND (request_cell_size flood), szn-specific:** the user's matrix test settled it — `mosh -- opencode` works, `mosh -- tmux + opencode` works, `ssh -t opencode` works, but `mosh -- szn + opencode` freezes. So it is NOT mosh; it is szn. Live client-log counters showed the client receiving ~9,587 `.request_cell_size` messages for only 12 output frames, and the server logged `sendRequestCellSize` firing **201,442 times**. The chain: opencode sends a DCS sequence that szn's parser treats as a sixel (`\x1bP…q`), which calls `addSixelImage` → sets `cell_size_needs_refresh`/`pending_sixel` → the server re-arms `needs_cell_size_refresh` on *every* pane read (sixel-wait, #204) and, since the client's `queryCellSize` costs a 5 ms stdin poll each, the flood froze the client before any real frame could be processed. tmux has no such sixel/request path, hence it works. Fixes: rate-limit `sendRequestCellSize` to 2/sec, and log the captured DCS bytes (`sixel DCS captured: len=N, preview=…`) so the exact trigger (false positive vs real sixel) can be identified.
