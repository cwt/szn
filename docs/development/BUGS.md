...
**Follow-up (ghost images persist after yazi / image replacement):** with yazi displaying and exiting, the terminal kept stale sixel overlays — ghosts re-appearing (in order) after exit, and old images left behind when the preview changed. Two causes: (1) exiting the alternate screen freed the marker cells but never freed the alt screen's `sixel_images` entries, so the renderer re-emitted them; (2) the renderer only erased on removed/moved images, so a same-anchor replacement (or any add) left the old overlay. Fix: `useAltScreen(false)` frees the alt screen's sixel images (`dropScreenSixels`), and `renderSixelImages` now erases-all + redraws every current-screen image on *any* change (added/removed/moved/replaced), filtering by `alt_screen` so the inactive screen's images are always cleared. Regression tests: alt-screen images freed on exit; replacement emits erase-all + only the new DCS.
