...
**Diagnostics gotcha:** the interactive client never loads the server config that enables logging, so every client-side diagnostic was invisible in `szn.log` (the log files were 100% server-side). Fixed `log.zig` to open the log in append mode (`O_APPEND`, no `O_TRUNC`) so the client can enable logging to the same file and interleave with the server; the client now calls `log_mod.enable("default")` at startup and logs backpressure on/off transitions, stdin/SIGHUP survival, and every exit path.
