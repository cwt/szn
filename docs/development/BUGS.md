...
**Follow-up (session unresponsive after img2sixel):** after a few img2sixel runs the session froze even though the server never spun (loop diag stayed at 1 iters/2s) and the client drained its output (out_buf 0). Suspect: the client's `awaiting_cell_size` stdin handling buffered any input starting with `\x1b[` as a "possible CSI 14 t reply"; with the server re-requesting cell size every 500 ms, escape-sequence input (arrow keys etc.) could be swallowed, making the session feel frozen. Fix: only buffer input that actually starts like a cell-size reply (`\x1b[4`); everything else forwards immediately. Added `awaiting_cell_size` to the client's periodic diagnostic to confirm/refute this on the next reproduction.
