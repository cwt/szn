...
tags: [sixel, graphics, architecture, grid]
...
2. **Terminal Erasure Quirks**: Sixel graphics are managed on a separate overlay layer by terminal emulators. Transparent space characters (`" "`) do not overwrite sixel pixels in most terminals, and `CSI X` (Erase Character) operates only on the text grid — it cannot clear the sixel overlay. To guarantee clean removal when an image scrolls or is hidden, `szn` tracks the last-drawn anchor of every registry slot (`Screen.sixel_last_anchor`) and, each frame, emits a DECSIXEL erase-below operation (`ESC P 1 q ESC \`) at the *previous* anchor for any image that moved or disappeared, *before* redrawing the still-visible images at their current anchors (see bug #195). This clears the overlay instead of leaving a smear/ghost as the image scrolls. Note that the sixel erase operation clears a rectangular region relative to the cursor, so (as with all terminal sixel implementations) an erase may also clear unrelated overlay pixels that happen to lie below/right of the old anchor.
